...
const columns = [
  ...
  },
  {
    ...
    key: 'enable',
    render(row) {
      return h('span', row.enable === '1' ? '启用' : '禁用')
    }
  },
  {
    title: '操作',
...
const handleTable = (row) => {
  ...
  modalForm.value.enable = row.enable || '1'
  handleModal('edit')
}
...
const handleEnable = (row) => {
  dialog.warning({
    title: '提示',
    content: `确定${row.enable === '1' ? '禁用' : '启用'}该账号`,
    positiveText: '确定',
    negativeText: '取消',
    onPositiveClick: () => {
      let reqApi = row.enable === '1' ? api.putUserDisable : api.putUserEnable
      reqApi(row.id).then(() => {
        message.success('操作成功')
        $table.value?.handleSearch()
      })
